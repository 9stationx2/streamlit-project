...
# 1) 페이지 & 헤더
# --------------------
st.set_page_config(page_title='Machine Learning Report', page_icon='💻')
...
df[num_cols] = imp_mean.fit_transform(df[num_cols]) # 각각의 컬럼에 대해서 결측치 평균값 들어감



# 2) 범주형 결측(성별)에 대해 최빈값 대체
...
report_df = pd.DataFrame(report_dict).T.round(2)  # 데이터 프레임 변환
st.subheader('Classification Report')
st.table(report_df)                               # 테이블 변환


# 8-3) Confusion Matrix : 값이 튀는 것에 대해 정확도 생각 해봐야 함
