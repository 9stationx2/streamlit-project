...
# 페이지설정
st.set_page_config(page_title='데이터시각화')
...
# 탭생성
t1,t2,t3,t4 = st.tabs(['BMI 분포 히스토그램', '성별 레이블 BMI 박스플롯', '레이블 카운트 막대그래프', '키 vs 몸무게 산점도'])
...
with t1 :
    ...
    st.plotly_chart(fig1, use_container_width=True, config={'responsive' : True})  # config={'responsive' : True}) : 반응형
...
# 키 vs 몸무게 산점도
with t4 :
    st.markdown('**키 vs 몸무게 산점도(체중 범주별)**')
    fig4 = go.Figure()
    for label in df['Label'].unique():
